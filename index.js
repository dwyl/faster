...
var script = './server.js'; // this will be set dynamically via process.argv
...
function runscript() {
  var run = false;
  child = exec('node '+script); // pass in the script here
    ...
    if(done === 0){
      ...
      uniquedirs.map(function(dir){
        ...
        fs.watch(dir, function (event, filename) {
          console.log(chalk.bgGreen.black(filename +" "+event));
          // console.log(" - - - - - - - - - ");
          // console.log(filename);
          if (filename) {
            // console.log('                                           filename provided: ' + filename);
            sio.emit(secret, "Latest: " + filename +" "+event);
            setTimeout(function(){
              kill(child.pid);
              return index();
            },10);
