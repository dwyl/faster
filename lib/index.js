...
console.log(chalk.red("PARENT Directory >> "+parent))
 ...
 *
 */
function checkisdir(fd, index, callback) {
  fs.stat(arg, function(err, stat) {
    if(err) {
      console.log(err)
      callback(err);
    }
    var file = arg.split('/')[arg.split('/').length-1]; // just the file/dir to lookup in ignore list
    // console.log(index +" | " + done +" | " + arg +" | "+fd +" | isFile: " + stat.isFile());
    if(stat.isDirectory() && ignore.indexOf(file) === -1) {
      dirs.push(arg);
      walk(arg, dirs, callback);
    }
...
function walk(dir, dirs, done, callback) {
  done++; // a count of how many dirs we still have to walk
...
function watchdirs(dirs, callback){
