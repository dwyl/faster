...
function isdir(fd, dirlist, callback, keepwalking, afterwalking) {
  fs.stat(fd, function(err, stat) {
    ...
    if(!err) {
      console.log("isdir >> "+fd + " | stat.isDirectory(): "+stat.isDirectory() +" |  counter: "+counter);
      ...
      if(stat.isDirectory() && ignore.indexOf(last) === -1) {
        ...
        console.log(' - - - - - -> KEEP on WALKING: '+counter);
      ...
      // don't add anything to the dirlist
    }
    counter--;
    if(counter === 0){
      console.log(chalk.bgGreen.black(' -> DONE WALKING count:'+counter + " | dirs:" + dirlist.length +" "));
...
var walk = function(dir, dirlist, afterwalking) {
  console.log(chalk.bgRed(" walk :" +dir + " | counter: "+ counter +" "))
  fs.readdir(dir, function(err,files) {
    var filecount = files.length;
    console.log(chalk.bgGreen.black(" filecount: "+filecount +" "));
    counter = counter + filecount; // a count of how many descriptors we still have to check
    if(filecount === 0 && counter === 0) {
      console.log(chalk.bgCyan.black(" EMPTY DIR! filecount: "+filecount +" | counter:"+counter
        + " | dirs:" + dirlist.length+" "));
      // counter = 1;
      // dirlist.push(dir);   // add the dir to the dirlist
      afterwalking(dirlist);
